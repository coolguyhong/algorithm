package graph.baekjoon.timemachine;
...
public class Main {
    ...
    private static void bellmanFordMoore() throws IOException {
        ...
        for (int j = 0; j < M; j++) {
            // 주어진 간선대로 한 번 더 가려고 하는데 기존 경로보다 작은 값이 나오면
            // 음수 사이클이 나와 갈 수가 없다.
            if (D[path[j][0]] != MAX_VALUE && D[path[j][1]] > D[path[j][0]] + path[j][2]) {
                bw.write("-1\n");
                bw.flush();
...
//    private static void bellmanFordMoore1() throws IOException {
//        D[1] = 0;
//        for (int i = 1; i <= N; i++) {
//            for (int j = 0; j < M; j++) {
//                if (D[path[j][0]] != MAX_VALUE && D[path[j][1]] > D[path[j][0]] + path[j][2]) {
